...
let billCalcInput = document.querySelector('.bill-calc__input').value;
let billCalcPeopleInput = document.querySelector('.bill-calc__people--input').value;
let billCalcInputCustom = document.querySelector('.bill-calc__input--custom').value;
...
const calcTip = (x, y, z) => {//function to calculate tip per person = billCalcInput * billPercentage * billCalcPeopleInput;
...
for (let i = 0; i < billCalcValue.length; i++) {// Looping through the array
  ...
  billCalcValue[i].addEventListener('click', function () { // Checking for clicks on button elements
    ...
    billCalcPeopleInput = Number(document.querySelector('.bill-calc__people--input').value);
    ...
    if (!billCalcPeopleInput) {//checks for input value, if input value is 0 or if there is no input value
        ...
        document.querySelector('.bill-calc__people--input').style.outline ='2px solid red'; //display outline of input field red
      ...
      if (billCalcValue[i] === billCalcValue[0]) { // ichecks if button clicked === first button of the array
        ...
        // Assign the value of calcTip function to the content of tipFigure & the value of calcTotal Function to the content of totalFigure
        tipFigure.textContent = `$${Number((calcTipValue * 3).toFixed(2))}`; //multiply value of calcTipValue by 3
        totalFigure.textContent = `$${calcTotal(Number((calcTipValue * 3).toFixed(2)), billCalcInput)}`; //multiply value of 
...
const func = function () {
  billCalcInput = Number(document.querySelector('.bill-calc__input').value);
  billCalcInputCustom = Number(document.querySelector('.bill-calc__input--custom').value) / 100;
  billCalcPeopleInput = Number(document.querySelector('.bill-calc__people--input').value);
};

document.querySelector('.bill-calc__input--custom').addEventListener('keyup', func);
document.querySelector('.bill-calc__people--input').addEventListener('keyup', function () {
    
    billCalcPeopleInput = Number(document.querySelector('.bill-calc__people--input').value);
    calcTipValue = calcTip(billCalcInput, billCalcPeopleInput, billCalcInputCustom);
...
btnReset.addEventListener('click', function () {//Resets the values of billCalcInput, billCalcPeopleInput, textcontent of tipFigure and totalFigure to 0.
  ...
  Number((document.querySelector('.bill-calc__input--custom').value = 'Custom'));
  tipFigure.textContent = '$0.00';
  totalFigure.textContent = '$0.00';
});